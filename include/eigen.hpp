...
std::tuple<Matrix, Matrix> eig_power(const Matrix& A, size_t n, double eps = 1E-5);
// Computes the n smallest eigen values (and asociated vectors).
// Requires symetric matrix A=At
std::tuple<Matrix, Matrix>
eig_inv_power(const Matrix& A, size_t n, SolveMethod method, double eps = 1E-5);
...
// Requires symetric matrix A=At
std::tuple<Matrix, Matrix>
eig_subspace_pow(const Matrix& A, size_t m, int iters, double eps = 1E-7, double jacobi_eps = 1E-3);
// Subspace iteration method for m smallest eigen values (and asociated vectors).
// Requires symetric matrix A=At
std::tuple<Matrix, Matrix> eig_subspace_inv(
  const Matrix& A, size_t m, int inv_iters, double eps = 1E-7, double jacobi_eps = 1E-3);
} // namespace ccm
